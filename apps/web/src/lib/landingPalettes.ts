import type { LandingThemeId } from '@abundance/shared';
 ...
 *
 * This table lives in its own module (rather than in landingTheme.ts) so the
 * link-preview image renderer at apps/web/api/og.tsx can import it without
 * pulling in zod or anything else from the app bundle — its only import here is
 * type-only, so it erases at build time. landingTheme.ts re-exports both names,
 * so nothing under src/ needs to know this file exists.
 */
